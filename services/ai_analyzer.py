...
class AIAnalyzer:
    ...
    async def get_futures_analysis(self, df: pd.DataFrame, futures_code: str, stream: bool = False) -> AsyncGenerator[str, None]:
        """
        对期货数据进行AI分析
        ...
        """
        try:
            logger.info(f"开始期货AI分析 {futures_code}, 流式模式: {stream}")
            ...
            volume_status = 'HIGH' if volume > volume_ma * 1.5 else ('LOW' if volume < volume_ma * 0.5 else 'NORMAL')
            
            # 确定持仓量状态
            open_interest = latest_data.get('OpenInterest', 0)
            open_interest_ma = latest_data.get('OI_MA', 0)
            open_interest_status = 'HIGH' if open_interest > open_interest_ma * 1.2 else ('LOW' if open_interest < open_interest_ma * 0.8 else 'NORMAL')
            ...
            technical_summary = {
                ...
                'volatility': f"{latest_data.get('VolatilityStd', 0):.2f}%",
                'volume_trend': 'increasing' if volume > volume_ma else 'decreasing',
                'open_interest_trend': 'increasing' if open_interest > open_interest_ma else 'decreasing',
                'rsi_level': rsi
            }
            ...
            请基于技术指标和期货市场特点进行分析，给出具体数据支持。分析中请特别关注持仓量变化与价格变化的关系，以及期货特有的交易特点。
            """
            ...
            async with httpx.AsyncClient(timeout=self.API_TIMEOUT) as client:
                # 记录请求
                logger.debug(f"发送期货AI请求: URL={api_url}, MODEL={self.API_MODEL}, STREAM={stream}")
                ...
                if stream:
                    ...
                    async with client.stream("POST", api_url, json=request_data, headers=headers) as response:
                        if response.status_code != 200:
                            ...
                            error_data = json.loads(error_text)
                            error_message = error_data.get('error', {}).get('message', '未知错误')
                            logger.error(f"期货AI API请求失败: {response.status_code} - {error_message}")
                            yield json.dumps({
                                "futures_code": futures_code,
                                "error": f"API请求失败: {error_message}",
                                "status": "error"
                        ...
                        logger.info(f"期货AI流式处理完成，共收到 {chunk_count} 个内容片段，总长度: {len(buffer)}")
                        ...
                        # 尝试从分析内容中提取投资建议
                        recommendation = self._extract_futures_recommendation(full_content)
                        
                        # 计算分析评分
                        score = self._calculate_futures_analysis_score(full_content, technical_summary)
                    ...
                    # 尝试从分析内容中提取投资建议
                    recommendation = self._extract_futures_recommendation(analysis_text)
                    
                    # 计算分析评分
                    score = self._calculate_futures_analysis_score(analysis_text, technical_summary)
            ...
            logger.error(f"期货AI分析出错: {str(e)}", exc_info=True)
            yield json.dumps({
                "futures_code": futures_code,
                "error": f"分析出错: {str(e)}",
                "status": "error"
    ...
    def _extract_futures_recommendation(self, analysis_text: str) -> str:
        """从期货分析文本中提取交易建议"""
        # 查找交易建议部分
        trading_advice_pattern = r"##?\s*交易建议\s*\n(.*?)(?:\n##?|\Z)"
        match = re.search(trading_advice_pattern, analysis_text, re.DOTALL)
        
        if not match:
            # 尝试查找投资建议部分
            trading_advice_pattern = r"##?\s*投资建议\s*\n(.*?)(?:\n##?|\Z)"
            match = re.search(trading_advice_pattern, analysis_text, re.DOTALL)
        ...
        if match:
            ...
            # 提取关键建议
            if "做多" in advice_text or "买入" in advice_text or "多头" in advice_text:
                return "做多"
            elif "做空" in advice_text or "卖出" in advice_text or "空头" in advice_text:
                return "做空"
            elif "持有" in advice_text:
                return "持有"
            else:
        ...
        # 如果没有找到明确的建议部分，尝试在整个文本中搜索关键词
        if "做多" in analysis_text or "买入" in analysis_text or "多头" in analysis_text:
            return "做多"
        elif "做空" in analysis_text or "卖出" in analysis_text or "空头" in analysis_text:
            return "做空"
        elif "持有" in analysis_text:
            return "持有"
    ...
    def _calculate_futures_analysis_score(self, analysis_text: str, technical_summary: dict) -> int:
        """计算期货分析评分"""
        score = 50  # 基础分数
        ...
        if technical_summary['open_interest_trend'] == 'increasing':
            score += 5
        else:
            score -= 5
        ...
        # 根据分析文本中的关键词调整分数
        if "强烈做多" in analysis_text or "显著上涨" in analysis_text:
            score += 20
        elif "做多" in analysis_text or "看涨" in analysis_text:
            score += 10
        elif "强烈做空" in analysis_text or "显著下跌" in analysis_text:
            score -= 20
        elif "做空" in analysis_text or "看跌" in analysis_text:
            score -= 10
